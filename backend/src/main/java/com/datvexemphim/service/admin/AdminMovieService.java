...
public class AdminMovieService {
    ...
    private void apply(Movie m, MovieUpsertRequest req) {
        ...
        // Resolve genre name from genreId
        if (req.genreId() != null) {
            movieGenreRepository.findById(req.genreId())
                    .ifPresent(g -> m.setGenre(g.getName()));
        }
        // Default active = true if not provided
        m.setActive(req.active() == null || Boolean.TRUE.equals(req.active()));
    }
