# 🧪 API Testing Documentation - DatVeXemPhim

> **Last updated**: 2026-03-20
> **Base URL**: `https://web-datvexemphim-production.up.railway.app/api`
> **Frontend**: `https://web-datvexemphim.vercel.app`
...
4. [Showtime APIs](#4-showtime-apis)
5. [Seat Map APIs](#5-seat-map-apis)
6. [Booking APIs](#6-booking-apis)
7. [Payment APIs](#7-payment-apis)
8. [Food & Drink APIs](#8-food--drink-apis)
9. [Voucher APIs](#9-voucher-apis)
10. [Movie Review APIs](#10-movie-review-apis)
...
```bash
curl -X POST "https://web-datvexemphim-production.up.railway.app/api/auth/forgot-password" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "testuser@example.com",
    "fullName": "Nguyễn Văn Test"
...
**Request:**
```bash
curl -X POST "https://webdatvexemphim-production.up.railway.app/api/auth/reset-password" \
  -H "Content-Type: application/json" \
  -d '{
    "token": "abc123...",
...
**Request:**
```bash
curl -X GET "https://webdatvexemphim-production.up.railway.app/api/user/profile" \
...
**Request:**
```bash
curl -X PUT "https://webdatvexemphim-production.up.railway.app/api/user/profile" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "fullName": "Admin Mới",
...
**Request:**
```bash
curl -X GET "https://web-datvexemphim-production.up.railway.app/api/movies/past"
```
...
**Request:**
```bash
curl -X GET "https://web-datvexemphim-production.up.railway.app/api/seatmap/20"
```
  ...
  [
    {
        "ticketId": 46,
        "status": "PENDING",
        "bookedAt": "2026-03-21T04:03:06.164240Z",
        "cancelledAt": null,
        "showtimeId": 20,
...
**Request:**
```bash
curl -X POST "https://web-datvexemphim-production.up.railway.app/api/tickets/10/cancel" \
  -H "Authorization: Bearer <token>"
```
...
**Request:**
```bash
curl -X GET "https://web-datvexemphim-production.up.railway.app/api/food/items/category/1"
```
...
**Request:**
```bash
curl -X DELETE "https://web-datvexemphim-production.up.railway.app/api/movies/reviews/2" \
  -H "Authorization: Bearer <token>"
```
...
```bash
curl -X GET "https://webdatvexemphim-production.up.railway.app/api/admin/movies" \
  -H "Authorization: Bearer <admin_token>"
```

#### POST `/api/admin/movies` - Tạo phim mới
```bash
curl -X POST "https://webdatvexemphim-production.up.railway.app/api/admin/movies" \
  -H "Authorization: Bearer <admin_token>" \
  -H "Content-Type: application/json" \
  -d '{
  "title": "Phim mới test1",
  "durationMinutes": 120,
  "posterUrl": "https://example.com/poster.jpg",
  "rating": "PG-13",
  "genreId": 10,
  "description": "Phim test từ API",
...
```bash
curl -X GET "https://web-datvexemphim-production.up.railway.app/api/admin/rooms" \
  -H "Authorization: Bearer <admin_token>"
```
...
```bash
curl -X GET "https://web-datvexemphim-production.up.railway.app/api/admin/showtimes" \
  -H "Authorization: Bearer <admin_token>"
```
...
```bash
curl -X GET "https://web-datvexemphim-production.up.railway.app/api/admin/vouchers" \
  -H "Authorization: Bearer <admin_token>"
```
...
```bash
curl -X GET "https://web-datvexemphim-production.up.railway.app/api/admin/food/categories" \
  -H "Authorization: Bearer <admin_token>"
```
...
```bash
curl -X GET "https://web-datvexemphim-production.up.railway.app/api/admin/food/items" \
  -H "Authorization: Bearer <admin_token>"
```
...
curl -X POST "https://web-datvexemphim-production.up.railway.app/api/admin/food/items" \
  ...
  -d '{
    "categoryId": 1,
    "name": "Bánh Pizza nhỏ",
    "description": "Pizza size nhỏ 6 inch",
    "price": 45000,
